        ...
        <div class="vue-row">
            <div class="main-content">
                <div class="vue-table">
                    ...
                    <transition name="fade" mode="out-in">
                        <div class="table-scroll scroll-box" v-if="subjects.length > 0">
                            <div class="vue-table__row" v-for="subject in subjects" >
                                <div class="vue-table__cell">
                                    {{ subject.name }}
                                </div>
                                <div class="vue-table__cell">
...
<script>
    ...
    export default {
        ...
        data: () => ({
            showNoMatch: false,
            showLoader: false,
            filterForm: localStorage.getItem('filterFormSubject')
                ? JSON.parse(localStorage.getItem('filterFormSubject'))
                : {
                    sort_field: '',
                    search: '',
                    search_field: '',
        ...
        methods:{
            ...
            getData(){
                ...
                axios
                    .post('/api/subject', this.filterForm).then(response => {
                    this.subjects = response.data;
                    if (this.subjects.length === 0) this.showNoMatch = true;
        ...
        watch: {
            filterForm: {
                handler(newValue) {localStorage.setItem("filterFormSubject", JSON.stringify(newValue))},
                deep: true
            },
            modals: {
                handler(newValue) {
